<?php
	...
	function recursive($source, $parent, &$newString){
		if(count($source) > 0) {
			...
			$newString .= '</ul>'; //kết thúc 
		}
	}
	

	recursive($menu, 0, $newString);
	$newString = str_replace('<ul></ul>', '', $newString); //xóa thẻ ul dư thừa
	echo $newString;
