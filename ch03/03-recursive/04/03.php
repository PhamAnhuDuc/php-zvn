<?php
	...
	foreach ($menu as $key => $value){
		if($value['parents'] == 0){  //khi duyệt cần tạo thêm thuộc tính lever 
			$value['level'] = 1;
			$newArray[] 	= $value;  //tạo mới 1 mảng
			unset($menu[$key]); //gắn vào mảng mới rồi thì thực hiện xóa nó ra khỏi mảng
			
			$parents = $value['id'];  //tạo ra 1 biến với value = id
			foreach($menu as $key_1 => $value_1){
				if($value_1['parents'] == $parents){
					$value_1['level'] = $value['level'] + 1;
					$newArray[] 	= $value_1;  //gắn vào mảng mới rồi thì thực hiện xóa nó ra khỏi mảng
					unset($menu[$key_1]);
					...
					foreach($menu as $key_2 => $value_2){
						if($value_2['parents'] == $parents_1){
							...
							$newArray[] 	= $value_2;
							unset($menu[$key_2]); //gắn vào mảng mới rồi thì thực hiện xóa nó ra khỏi mảng
						}
					}
				}
			}
